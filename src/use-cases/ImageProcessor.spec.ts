...
describe('Image Processor Use Case', () => {
  ...
  it('should process successfully an image from upload', async () => {
    ...
    const image = await imageUseCase.processImageFromUpload(
      fs.readFileSync(
        path.resolve(__dirname, '../tests/fixtures/dummy-image.jpeg'),
      ),
    )
  ...
  it('should throw an error when trying to process an image that does not exist', async () => {
    ...
    try {
      await imageUseCase.processImageFromUrl(
        'https://images.pexels.com/photos/14241230490/pexels-photo-14240490.jpeg',
      )
    } catch (error: any) {
      expect(error.message).toBe(
        'Error processing image from URL https://images.pexels.com/photos/14241230490/pexels-photo-14240490.jpeg: Request failed with status code 404',
      )
    }
  })
